...
PATENT_TITLE = "一种基于双视图对比学习的鲁棒异构图节点分类方法"
SHORT_TITLE = "DVCL鲁棒异构图节点分类方法"
...
def add_process_step(doc, marker, text):
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    p.paragraph_format.left_indent = Cm(1.45)
    p.paragraph_format.first_line_indent = Cm(-1.45)
    p.paragraph_format.tab_stops.add_tab_stop(Cm(1.45))
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(0)
...
def build_document(template_path, output_path):
    ...
    with tempfile.TemporaryDirectory(prefix="dvcl_patent_figures_") as temp_dir:
        ...
        add_body(
            doc,
            "本发明涉及人工智能、图机器学习和数据安全技术领域，具体涉及一种在异构图的边关系可能受到恶意插入、删除或其他结构扰动时，利用净化语义拓扑视图与特征诱导视图进行双视图表示学习、跨视图对比约束和节点分类的方法。",
        )
        ...
        add_process_step(doc, "S1.", "获取异构图、节点类型、关系类型、目标节点特征及训练节点标签；确定待分类的目标节点类型和元路径集合。")
        add_process_step(doc, "S2.", "沿各元路径组合关系邻接矩阵，得到多个目标节点同构语义图；计算特征校准边置信度，并执行元路径级第一阶段过滤。")
        add_process_step(doc, "S3.", "在净化元路径图上学习语义注意力权重，对各元路径的边置信度进行加权合并，并执行全局第二阶段过滤，得到净化拓扑视图。")
        add_process_step(doc, "S4.", "对目标节点特征进行归一化和相似度计算，为每个节点选择k个近邻，得到特征诱导视图。")
        add_process_step(doc, "S5.", "利用两个独立图编码器分别编码净化拓扑视图和特征诱导视图，得到按节点对应的两组表示。")
        add_process_step(doc, "S6.", "融合两组表示并输出节点分类结果；在训练阶段计算监督分类损失、辅助语义分支损失和对称跨视图对比损失。")
        add_process_step(doc, "S7.", "根据总损失反向传播并更新模型参数；根据验证节点上的预设指标保存最佳参数组合。")
        add_process_step(doc, "S8.", "在推理阶段恢复最佳参数，重新构造或读取两个视图，输出目标节点类别及其置信度。")
        ...
        add_heading(doc, "实施例三：训练与推理过程", 2)
        add_body(
            doc,
            "本实施例包括离线训练和在线或批量推理两个阶段。离线训练阶段读取异构图快照，生成元路径矩阵、净化拓扑视图和特征诱导视图，联合训练语义注意力模块、两个图编码器和分类器，并将最佳参数、元路径定义、过滤阈值和近邻参数作为同一模型版本保存。",
        )
        add_body(
            doc,
            "推理阶段加载对应模型版本。对于静态快照，可复用已保存的两个视图；对于新增节点或周期性更新的异构图，可重新计算受影响的元路径候选边和特征近邻，随后执行双视图编码和融合分类。输出可以包括预测类别、类别概率、两个视图的单独诊断分数以及门控融合权重，以支持下游风险分析。",
        )
        add_body(
            doc,
        ...
        add_numbered(doc, "6.", "近邻模式、过滤规则、图编码器和融合方式的等同替换方案。")
        ...
        add_body(
            doc,
            "建议从属权利要求依次限定：元路径矩阵连乘；特征校准边置信度；元路径级阈值；语义注意力权重；全局第二阶段过滤；有向、对称或互为近邻建图；带自环的独立图编码器；特征遮蔽；拼接或门控融合；对称InfoNCE；辅助语义分类损失；最佳检查点恢复。",
        )
