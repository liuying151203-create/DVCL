...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Prepare model-independent adaptive target attack requests."
    )
    ...
    parser.add_argument("--output-root", default=str(ROOT / "data" / "attacks"))
...
def stratified_targets(candidates, labels, count, seed):
    candidates = np.asarray(
        sorted(set(int(value) for value in candidates)), dtype=np.int64
    )
...
def prepare_requests(args) -> int:
    ...
    for dataset in args.datasets:
        ...
        targets = stratified_targets(
            candidates, clean.labels.numpy(), args.target_count, args.target_seed
        )
        ...
        for seed in args.seeds:
            for rate in args.rates:
                output = (
                    output_root / dataset / "adaptive_query"
                ...
                artifact = build_attack_artifact(
                    clean, split, "adaptive_query", rate, seed,
                    clean.hete_adjs, targets, str(hg_path.resolve()),
                    file_sha256(hg_path),
                    provenance={
                        "generator": "scripts/prepare_adaptive_requests.py",
                        "request_only": True,
...
def main() -> int:
    return prepare_requests(parse_args())
